...
const gem_bank_client_1 = require("./gem-bank.client");
...
class GemFarmClient extends gem_bank_client_1.GemBankClient {
    ...
    fetchTreasuryBalance(farm) {
        return __awaiter(this, void 0, void 0, function* () {
            const [treasury] = yield this.findFarmTreasuryPDA(farm);
            return this.getBalance(treasury);
        });
    }
    // --------------------------------------- find PDA addresses
    findFarmerPDA(farm, identity) {
        return __awaiter(this, void 0, void 0, function* () {
    ...
    findAuthorizationProofPDA(farm, funder) {
        return __awaiter(this, void 0, void 0, function* () {
            return this.findProgramAddress(this.farmProgram.programId, [
                'authorization',
                farm,
    ...
    findRewardsPotPDA(farm, rewardMint) {
        return __awaiter(this, void 0, void 0, function* () {
            return this.findProgramAddress(this.farmProgram.programId, [
                'reward_pot',
                farm,
    ...
    farmConfig) {
        return __awaiter(this, void 0, void 0, function* () {
            const [farmAuth, farmAuthBump] = yield this.findFarmAuthorityPDA(farm.publicKey);
            const [farmTreasury, farmTreasuryBump] = yield this.findFarmTreasuryPDA(farm.publicKey);
            const [rewardAPot, rewardAPotBump] = yield this.findRewardsPotPDA(farm.publicKey, rewardAMint);
            const [rewardBPot, rewardBPotBump] = yield this.findRewardsPotPDA(farm.publicKey, rewardBMint);
            ...
            const txSig = yield this.farmProgram.rpc.initFarm(farmAuthBump, farmTreasuryBump, rewardAPotBump, rewardBPotBump, rewardAType, rewardBType, farmConfig, {
                accounts: {
                    farm: farm.publicKey,
    ...
    updateFarm(farm, farmManager, config = null, newManager = null) {
        return __awaiter(this, void 0, void 0, function* () {
            const signers = [];
            if ((0, gem_common_1.isKp)(farmManager))
                signers.push(farmManager);
            console.log('updating farm');
            const txSig = yield this.farmProgram.rpc.updateFarm(config, newManager, {
                accounts: {
                    farm,
                    farmManager: (0, gem_common_1.isKp)(farmManager)
                        ? farmManager.publicKey
    ...
    payoutFromTreasury(farm, farmManager, destination, lamports) {
        return __awaiter(this, void 0, void 0, function* () {
            const [farmAuth, farmAuthBump] = yield this.findFarmAuthorityPDA(farm);
            const [farmTreasury, farmTreasuryBump] = yield this.findFarmTreasuryPDA(farm);
            const signers = [];
            if ((0, gem_common_1.isKp)(farmManager))
                signers.push(farmManager);
    ...
    addToBankWhitelist(farm, farmManager, addressToWhitelist, whitelistType) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const [farmAuth, farmAuthBump] = yield this.findFarmAuthorityPDA(farm);
            const [whitelistProof, whitelistProofBump] = yield this.findWhitelistProofPDA(farmAcc.bank, addressToWhitelist);
            const signers = [];
            if ((0, gem_common_1.isKp)(farmManager))
                signers.push(farmManager);
            console.log(`adding ${addressToWhitelist.toBase58()} to whitelist`);
            const txSig = yield this.farmProgram.rpc.addToBankWhitelist(farmAuthBump, whitelistProofBump, whitelistType, {
                accounts: {
                    farm,
                    farmManager: (0, gem_common_1.isKp)(farmManager)
                        ? farmManager.publicKey
    ...
    removeFromBankWhitelist(farm, farmManager, addressToRemove) {
        return __awaiter(this, void 0, void 0, function* () {
            const farmAcc = yield this.fetchFarmAcc(farm);
            const [farmAuth, farmAuthBump] = yield this.findFarmAuthorityPDA(farm);
            const [whitelistProof, whitelistProofBump] = yield this.findWhitelistProofPDA(farmAcc.bank, addressToRemove);
            const signers = [];
            if ((0, gem_common_1.isKp)(farmManager))
                signers.push(farmManager);
    ...
    initFarmer(farm, farmerIdentity, payer) {
        return __awaiter(this, void 0, void 0, function* () {
            const identityPk = (0, gem_common_1.isKp)(farmerIdentity)
                ...
                : farmerIdentity;
            const farmAcc = yield this.fetchFarmAcc(farm);
            const [farmer, farmerBump] = yield this.findFarmerPDA(farm, identityPk);
            const [vault, vaultBump] = yield this.findVaultPDA(farmAcc.bank, identityPk);
            const [vaultAuth, vaultAuthBump] = yield this.findVaultAuthorityPDA(vault); //nice-to-have
            const signers = [];
            ...
            const txSig = yield this.farmProgram.rpc.initFarmer(farmerBump, vaultBump, {
                accounts: {
                    farm,
                    farmer,
                    identity: identityPk,
    ...
    stakeCommon(farm, farmerIdentity, unstake = false) {
        return __awaiter(this, void 0, void 0, function* () {
            const identityPk = (0, gem_common_1.isKp)(farmerIdentity)
                ? farmerIdentity.publicKey
                : farmerIdentity;
            ...
            if (unstake) {
                ...
                txSig = yield this.farmProgram.rpc.unstake(farmAuthBump, farmTreasuryBump, farmerBump, {
                    accounts: {
                        farm,
                        farmer,
                        farmTreasury,
    ...
    claim(farm, farmerIdentity, rewardAMint, rewardBMint) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const [farmAuth, farmAuthBump] = yield this.findFarmAuthorityPDA(farm);
            const [farmer, farmerBump] = yield this.findFarmerPDA(farm, identityPk);
            const [potA, potABump] = yield this.findRewardsPotPDA(farm, rewardAMint);
            const [potB, potBBump] = yield this.findRewardsPotPDA(farm, rewardBMint);
    ...
    flashDeposit(farm, farmerIdentity, gemAmount, gemMint, gemSource, mintProof, metadata, creatorProof) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const [vault, vaultBump] = yield this.findVaultPDA(farmAcc.bank, identityPk);
            const [farmAuth, farmAuthBump] = yield this.findFarmAuthorityPDA(farm);
            const [gemBox, gemBoxBump] = yield this.findGemBoxPDA(vault, gemMint);
            const [GDR, GDRBump] = yield this.findGdrPDA(vault, gemMint);
            const [vaultAuth, vaultAuthBump] = yield this.findVaultAuthorityPDA(vault);
            const [gemRarity, gemRarityBump] = yield this.findRarityPDA(farmAcc.bank, gemMint);
            ...
            const txSig = yield this.farmProgram.rpc.flashDeposit(farmerBump, vaultAuthBump, gemBoxBump, GDRBump, gemRarityBump, gemAmount, {
                accounts: {
                    farm,
                    farmAuthority: farmAuth,
                    farmer,
    ...
    authorizeCommon(farm, farmManager, funder, deauthorize = false) {
        return __awaiter(this, void 0, void 0, function* () {
            const [authorizationProof, authorizationProofBump] = yield this.findAuthorizationProofPDA(farm, funder);
            const signers = [];
            if ((0, gem_common_1.isKp)(farmManager))
                signers.push(farmManager);
            let txSig;
            if (deauthorize) {
                ...
                txSig = yield this.farmProgram.rpc.authorizeFunder(authorizationProofBump, {
                    accounts: {
                        farm,
                        farmManager: (0, gem_common_1.isKp)(farmManager)
                            ? farmManager.publicKey
    ...
    fundReward(farm, rewardMint, funder, rewardSource, variableRateConfig = null, fixedRateConfig = null) {
        return __awaiter(this, void 0, void 0, function* () {
            const funderPk = (0, gem_common_1.isKp)(funder)
                ...
                : funder;
            const [farmAuth, farmAuthBump] = yield this.findFarmAuthorityPDA(farm);
            const [authorizationProof, authorizationProofBump] = yield this.findAuthorizationProofPDA(farm, funderPk);
            const [pot, potBump] = yield this.findRewardsPotPDA(farm, rewardMint);
            const signers = [];
    ...
    addRaritiesToBank(farm, farmManager, rarityConfigs) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const config of completeRarityConfigs) {
                const [gemRarity] = yield this.findRarityPDA(bank, config.mint);
                //add mint
                remainingAccounts.push({
                    pubkey: config.mint,
                    isWritable: false,
